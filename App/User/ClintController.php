...
class ClintController extends Controller
{
	...
	{
		$total=(new Pessoa())->find("tipo=:t","t=Cliente")->fetch(true);
		$activos=(new Pessoa())->find("tipo=:t AND status=:s","t=Cliente&s=Activo")->fetch(true);
		$inativo=(new Pessoa())->find("tipo=:t AND status=:s","t=Cliente&s=bloq")->fetch(true);

        $this->engine->render("cliente/home",
           [
        	"listar"=>(new Pessoa())->find("tipo=:t AND status=:s","t=Cliente&s=Activo")->fetch(true),"sit"=>null,
     ...
     public function inativo()
	{
        ...
        $this->engine->render("login/inativo",["listar"=>(new Pessoa())->find("tipo=:t AND status=:s","t=Cliente&s=bloq")->fetch(true),"sit"=>"Inativo"]);
	}
	...
	public function store($data)
	{
        ...
        $this->engine->render("cliente/cadastro",["user"=>$data]);
	}
	...
	public function SaveData($data)
	{
		...
		if (!isset($data["entidade"])) {
			$js["warning"]="Indique o Tipo de Cliente";
			echo json_encode($js);
			return ;
		}
		...
		if (empty($data["id_pessoa"])) {
			$ps->sexo=(isset($data["sexo"]) ? $data["sexo"] : 'Indefinido');
		}
		$ps->bi=$data["bi"];
	...
	public function del($data)
	{
		...
		echo json_encode([
			"success"=> $this->view->v("cliente/tblpsooa",["listar"=>$data]),
			"total"=>isset($total) ? count($total) : 0,
        	"activos"=>isset($activos) ? count($activos) : 0,
        	"inativo"=> isset($inativo) ? count($inativo) : 0
		]);
